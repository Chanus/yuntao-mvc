	...
	var verify = {
		...
		checkDataLength : function(value, item) {
			...
			if (!verify.isNull(dataLength)) {
				...
				return lens[1] === undefined ? (value.length == parseInt(lens[0]) ? true : false) : ((value.length >= parseInt(lens[0]) && value.length <= parseInt(lens[1])) ? true : false);
			}
			return true;
		},
		checkNumberDigits : function(value, item) {
			...
			if (!verify.isNull(integerDigits)) {
				...
				b = digits[1] === undefined ? (i.length == parseInt(digits[0]) ? true : false) : ((i.length >= parseInt(digits[0]) && i.length <= parseInt(digits[1])) ? true : false);
			}
			...
			if (!verify.isNull(fractionDigits)) {
				...
				b = digits[1] === undefined ? (len == parseInt(digits[0]) ? true : false) : ((len >= parseInt(digits[0]) && len <= parseInt(digits[1])) ? true : false);
			}
