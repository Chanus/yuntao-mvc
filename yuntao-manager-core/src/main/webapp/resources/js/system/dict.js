...
layui.config({
    ...
    table.on('tool(datas)', function (obj) {
        ...
        if (layEvent === 'update') {// 编辑
            popup.open(600, 500, '<i class="layui-icon layui-icon-edit"></i>编辑数据字典集', action_path + 'update.do?id=' + data.id);
        } else if (layEvent === 'del') {// 删除
            operations.del({ids: [data.id]}, action_path + 'delete.do');
        } else if (layEvent === 'dictItem') {// 字典项
            popup.open(900, 600, '<i class="layui-icon layui-icon-set"></i>字典集【' + data.dictCode + '】字典项', action_path + 'item/main.do?dictCode=' + data.dictCode);
        }
    });
