...
layui.config({
        ...
        size: 20,// 限制20kb
            ...
            if (res.code === 0) {
                layer.msg(res.msg, {icon: 1, time: 1000}, function () {
                    $('#userFace').attr('src', parentCtx + res.data);
                });
            } else {
    ...
    form.on('submit(update)', function (data) {
        $.post(action_path + 'operator-info.do', data.field, function (data) {
            if (data.code === 0) {
    ...
    form.on('submit(updatePwd)', function (data) {
        $.post(action_path + 'update-own-password.do', data.field, function (data) {
            if (data.code === 0) {
                layer.msg(data.msg, {icon: 1, time: 1000}, function () {
                    window.location.reload();
                });
