...
layui.config({
    ...
    table.render({
        ...
        cols: [[
            ...
            {field: 'loginFlag', title: '登录系统', width: '10%', unresize: true, align: 'center', templet: '#loginFlagTpl'},
            {field: 'hasOperator', title: '具有操作员', width: '10%', unresize: true, align: 'center', templet: '#hasOperatorTpl'},
    ...
    table.on('tool(datas)', function (obj) {
        ...
        if (layEvent === 'update') {// 编辑
            popup.open(600, 650, '<i class="layui-icon layui-icon-edit"></i>编辑角色', action_path + 'update.do?id=' + data.id);
        } else if (layEvent === 'del') {// 删除
            operations.del({ids: [data.id]}, action_path + 'delete.do', init);
        }
    });
    ...
    // 渲染角色菜单权限列表
    renderRoleModulePower = function (roleId) {
    ...
    callback: {
        ...
        onClick: function (event, treeId, treeNode) {
            if (treeNode.level === 0) {// 点击根节点重新加载角色列表
                ...
                renderRoleModulePower(treeNode.id, treeNode.pId);
            }
        }
    },
    data: {
