...
layui.config({
    ...
    table.on('tool(datas)', function (obj) {
        ...
        if (layEvent === 'update') {// 编辑
            popup.open(600, 650, '<i class="layui-icon layui-icon-edit"></i>编辑子账号', action_path + 'update.do?id=' + data.id);
        } else if (layEvent === 'password') {// 修改密码
            popup.open(600, 300, '<i class="layui-icon layui-icon-edit"></i>修改密码', action_path + 'password.do?subNo=' + data.operatorNo);
