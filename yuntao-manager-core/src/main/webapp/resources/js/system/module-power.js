...
layui.config({
	...
	$("#add").on('click', function() {
		...
		$.post(action_path + 'add.do', {moduleId: moduleId, powerItem: powerItem, aliasName: powerName}, function(data) {
			layer.close(loading);
			if (data.code == 0) {
	...
	table.on('tool(datas)', function(obj) {
		...
		if (layEvent === 'update') {// 编辑
			popup.open(500, 300, '<i class="layui-icon layui-icon-edit"></i>编辑模块权限项', action_path + 'update.do?mpId=' + data.mpId);
		} else if (layEvent === 'configure') {// 配置模块权限项请求URL
			popup.open(800, 500, '<i class="layui-icon layui-icon-set-fill"></i>配置【' + data.moduleName + '-' + data.aliasName + '】请求URL', action_path + 'method-main.do?mpId=' + data.mpId);
		} else if (layEvent === 'del') {// 删除
			operations.del({ids: [data.mpId]}, action_path + 'delete.do');
		}
	});
