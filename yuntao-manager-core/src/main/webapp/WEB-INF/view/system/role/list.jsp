				...
				<tr>
					...
					<td align="left">
						{{# for(var k = 0; k < d[i].children[j].modulePowers.length; k++){ }}
							<input type="checkbox" id="module_power_{{ d[i].children[j].moduleId }}_{{ d[i].children[j].modulePowers[k].mpId }}" value="{{ d[i].children[j].moduleId }}_{{ d[i].children[j].modulePowers[k].powerItem }}" {{# if(d[i].children[j].modulePowers[k].hasPower){ }}checked{{# } }} style="width:16px;height:16px;">{{ d[i].children[j].modulePowers[k].aliasName }}
						{{# } }}
					</td>
