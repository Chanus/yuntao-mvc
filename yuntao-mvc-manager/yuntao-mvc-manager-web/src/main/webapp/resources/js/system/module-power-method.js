...
layui.config({
    ...
    $('#add').on('click', function () {
        ...
        $.post(action_path + 'method-add.do', {mpId: mpId, className: className, url: url}, function (data) {
            layer.close(loading);
            if (data.code === 0) {
    ...
    table.on('tool(datas)', function (obj) {
        ...
        if (layEvent === 'del') {// 删除
            operations.del({ids: [data.mpmId]}, action_path + 'method-delete.do', function () {
                reload();
            });
        }
    });
    ...
    table.on('edit(datas)', function (obj) {
        ...
        $.ajax({
            type: 'post',
            url: action_path + 'method-update.do?mpmId=' + data.mpmId + '&' + field + '=' + value,
            dataType: 'json',
            success: function (data) {
                if (data.code === 0) {
    ...
    // 增删改
    operations.config({path: action_path, id: 'mpmId', delUrl: 'method-delete.do', delFunc: function () {
        reload();
    }});
});